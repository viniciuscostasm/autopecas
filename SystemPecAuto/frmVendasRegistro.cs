    ...
    public partial class frmVendasRegistro : Form
    {
        ...
        private void btnObterDados_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(invoiceNo) as [Fatura No.],CONVERT(DateTime,InvoiceDate,105) as [Data Fatura],RTRIM(SubTotal) as [SubTotal],RTRIM(VATPer) as [Impostos %],RTRIM(VATAmount) as [Valor Impostos],RTRIM(DiscountPer) as [Desconto %],RTRIM(DiscountAmount) as [Valor Desconto],RTRIM(GrandTotal) as [Total],RTRIM(Cash) as [Dinheiro],RTRIM(Change) as [Troco] from Invoice_Info where InvoiceDate between @d1 and @d2 order by InvoiceDate", con);
        ...
        private void btnExportarExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Não há dados para exportar...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int rowsTotal = 0;
            int colsTotal = 0;
